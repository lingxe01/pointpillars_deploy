...
__global__ void make_pillar_histo_kernel(
    const float *dev_points,                                                         // 输入点云 [N, 5]
    float *dev_pillar_point_feature_in_coors,                                        // 输出，每个pillar中点的特征 [H, W, N, D]展平
    int *pillar_count_histo,                                                         // 输出：每个pillar中的点数 [H * W]
    const int num_points,                                                            // 点数N
    const int max_points_per_pillar,                                                 // 每个pillar允许的最多点数
    const int grid_x_size, const int grid_y_size, const int grid_z_size,             // pillar网格大小（XY分辨率） grid_z_size = 1
    const float min_x_range, const float min_y_range, const float min_z_range,       // 激光最小坐标
    const float pillar_x_size, const float pillar_y_size, const float pillar_z_size, // 每个pillar的宽度
    const int num_point_feature) // 每个点的维度，通常是5（x,y,z,intensity,0）
{ 

    // 计算当前线程要处理的点索引
    int th_i = blockIdx.x * blockDim.x + threadIdx.x;
    if (th_i >= num_points) return; // 超过点数量则直接退出

    // 将点的 (x, y, z) 坐标映射到 pillar 网格坐标 (x_coor, y_coor, z_coor)
    ...
    int max_points_pre_pillar)
{
    ...
    unsigned int idx_pre = ith_pillar * max_points_pre_pillar * num_point_feature + ith_point * num_point_feature;
    unsigned int idx_post = ith_pillar * max_points_pre_pillar * num_point_feature + (ith_point + blockDim.x) * num_point_feature;
    ...
    float *dev_pfe_gather_feature_)
{
    ...
    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 0] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 0];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 1] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 1];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 2] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 2];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 3] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 3];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 4] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 4];
    ...
    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 5] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 0] - dev_points_mean[ith_pillar * 3 + 0];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 6] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 1] - dev_points_mean[ith_pillar * 3 + 1];

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 7] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 2] - dev_points_mean[ith_pillar * 3 + 2];

    // f_center[:, :, 0] = voxel_features[:, :, 0] - (coords[:, 3].to(voxel_features.dtype).unsqueeze(1) * self.voxel_x + self.x_offset)
    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 8] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 0] - (dev_pillar_coors[ith_pillar * 4 + 3] * pillar_x_size + (pillar_x_size / 2 + min_x_range)); // min_x_range 最小范围值 -51.2

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 9] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 1] - (dev_pillar_coors[ith_pillar * 4 + 2] * pillar_y_size + (pillar_y_size / 2 + min_y_range));

    dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar * num_gather_feature + ith_point * num_gather_feature + 10] = dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar * num_point_feature + ith_point * num_point_feature + 2] - (dev_pillar_coors[ith_pillar * 4 + 1] * pillar_z_size + (pillar_z_size / 2 + min_z_range));
}
...
PreprocessPointsCuda::PreprocessPointsCuda(
    ...
    : num_threads_(num_threads),
      ...
      max_num_points_per_pillar_(max_points_per_pillar), // 20
